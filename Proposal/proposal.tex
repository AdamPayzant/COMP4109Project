...
\begin{document}
    ...
    This project is an implementation of a secure text, video and audio messaging app 
    (all media will be generalized as ``messages''). The primary objective is to provide a secure and 
    convenient way to send text messages and video calls while not having to worry about message contents being breached. 
    This has historically been a challenge, as it is difficult to guarantee transmission safety in the average modern network, 
    as well as storage safety for received messages. Many attempts at solving these problem exist such as SMS, Apple's iMessage, 
    WhatsApp and Signal. However, the majority of these solutions employ some form of strategy where messages are encrypted, 
    passed to a central server, and then finally sent to the destination. All of these solutions have notable failings or 
    prioritized convenience over security. Failings and compromises such as, 3rd parties storing messages and or breaking 
    end to end encryption. As a result, we've opted to use peer-to-peer transmission with end-to-end encryption to maximize security. 
